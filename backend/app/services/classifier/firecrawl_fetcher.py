                ...
                async with session.post(scrape_url, headers=headers, json=payload) as response:
                    if response.status != 200:
                        ...
                        is_quota_error = (
                            ...
                            any(phrase in error_text.lower() for phrase in [
                                'rate limit', 'quota', 'exceeded', 'too many requests', 'credits'
                            ])
                        )

                        if is_quota_error:
                            result['error'] = f"Firecrawl quota/rate limit exceeded (HTTP {response.status})"
                            logger.error(f"Firecrawl quota/rate limit exceeded: {error_text[:200]}")
                            ApiTracker.track_firecrawl(
                                success=False,
                                error_message=f"QUOTA/RATE LIMIT: {error_text[:400]}"
                            )
                        else:
                            result['error'] = f"Firecrawl API error: {response.status}"
                            logger.error(f"Firecrawl API error {response.status}: {error_text}")
                            ApiTracker.track_firecrawl(success=False, error_message=result['error'])
                    ...
                    if not data.get('success'):
                        ...
                        is_quota_error = any(phrase in error_msg.lower() for phrase in [
                            'rate limit', 'quota', 'exceeded', 'credits', 'limit reached'
                        ])

                        if is_quota_error:
                            logger.error(f"Firecrawl quota/rate limit in response: {error_msg}")
                            ApiTracker.track_firecrawl(
                                success=False,
                                error_message=f"QUOTA/RATE LIMIT: {error_msg[:400]}"
                            )
                        else:
                            ApiTracker.track_firecrawl(success=False, error_message=error_msg[:500])
                    ...
                    ApiTracker.track_firecrawl(success=True)
